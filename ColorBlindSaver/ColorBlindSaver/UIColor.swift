...
//  ColorBlindSaver
//
//  Created by Chunxi Ding on 1/30/19.
...
extension UIColor {
    ...
    func colorname() -> String? {
        ...
        if self.getRed(&fRed, green: &fGreen, blue: &fBlue, alpha: &fAlpha) {
            ...
            let rgbstring = String(iRed) + ", " + String(iGreen) + ", " + String(iBlue)
            
            //let hex = NSString(format:"#%06x", rgb)
            //return rgb
            
            var nearest = -1
            var result = "Unknown"
            for name in names {
                if (iRed == name.value.0 && iGreen == name.value.1 && iBlue == name.value.2) {
                    result = name.key
                    return result + "(" + rgbstring + ")"
                }
                
                let dist = (iRed-name.value.0)*(iRed-name.value.0) + (iGreen-name.value.1)*(iGreen-name.value.1) + (iBlue-name.value.2)*(iBlue-name.value.2)
